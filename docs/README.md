...
- repo：21 個 migration（第 21 個為單位通行碼登入節流，2026-08-22 新增，**尚未套用至遠端**）；遠端仍為 20 個。
- 遠端：47 張 `public` tables，其中 41 張為 `reibi_*`。
- Python：3,942 tests；pgTAP：170 項（2026-08-22 於本機空資料庫全量重播 21 個 migration 後通過，`db:lint` 無錯誤）；TypeScript 與 production build 最近一次均已通過。
