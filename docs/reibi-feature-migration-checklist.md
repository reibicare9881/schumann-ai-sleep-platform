...
- [x] FND-06 重要寫入操作 audit log —— 2026-08-20 完成，見 §43。
...
- [-] MP-06H 隱私、資安文件、稽核紀錄與版本／操作說明已整合；方案與定價頁已於 2026-08-20 完成（`/reibi/pricing`，見 §40）；**關於 REIBI 五分頁結構已完成**（`/about-reibi`），其中 SDG 與 888 計畫已定稿，**使命／理論框架／利害關係人三頁為佔位文案**，見 §41。
...
- [~] MP-06K 體驗場域（Artifact `VenueScreen`）：**結構已完成**（`/venues`、`reibi_experience_venues`、首次免費額度表），**內容為明確標示的佔位資料**，正式場域資料待業務端提供。見 §41。
...
- [~] MP-06K：`VenueScreen` 體驗場域**結構已於 2026-08-20 完成**（見 §41），內容為明確標示的佔位資料；正式場域資料待業務端提供。首次免費體驗的「每人限一次」已實作並由資料庫主鍵保證。
...
## 41. Batch S11 體驗場域與關於 REIBI（2026-08-20）

依使用者要求「先生成模擬的」，因此**結構完成、內容為佔位**。核心設計原則：
**佔位內容必須一眼看得出是佔位，而且不能產生真實後果。**

### 體驗場域

- [x] VEN-S01：新增 `reibi_experience_venues`（第 20 個 migration）。**不沿用 `reibi_enterprise_sites`** ——
  那張表是 `enterprise_id not null`，代表客戶自己的廠區；體驗中心是 REIBI 自有據點，
  不屬於任何企業，塞進去會讓租戶範圍失去意義。
- [x] VEN-S02：**佔位場域不可被預約**（`is_placeholder` → `bookable: false`）。
  編造的地址若上線，會有人真的照著跑一趟。此判斷放在後端 `normalise_venue` 單一位置，
  前端與預約 API 都讀它，不各自解讀。
- [x] VEN-S03：**首次免費體驗額度由資料庫主鍵保證**，不靠查詢聚合。
  新增 `reibi_venue_free_visits`，主鍵僅 `profile_id`，因此「每人限一次」是結構性事實 ——
  兩個並發請求可以同時通過「查一下有沒有用過」的檢查，主鍵不會。
  Artifact 把「每人限一次」寫在畫面上但**從未檢查過額度**。
- [x] VEN-S04：額度範圍為「每人一次，不分場域」，**2026-08-20 經業務端確認**（此前為依 Artifact 字面
  推定的假設）。實作早已是此規則，本次僅將紀錄由假設改為確認。若日後改為「每人每場域一次」，
  將 `reibi_venue_free_visits` 主鍵改為 `(profile_id, venue_id)` 即可 ——
  那是一次資料庫層級的規則變更，不是散落在應用程式各處的 if。
- [ ] VEN-S05：**正式場域資料待提供** —— 每家需要：名稱、完整地址、所在區域、交通方式（可多條）、
  開放時間、體驗項目、備註、是否開放首次免費。

### 關於 REIBI

- [x] ABT-S01：`/about-reibi` 五分頁上線。每個分頁帶 `is_placeholder` 旗標，
  前端據此顯示警示 —— 不是靠讀者自己看出文案是假的。
- [x] ABT-S02：**888 計畫由 `PLAN_888_TIMELINE` 產生**，與組織分析的 888 進度使用同一份定義。
  照抄一份會出現「文件說第 5 週、系統算第 6 週」的落差。
- [x] ABT-S03：SDG 對標沿用 Artifact 既有文字（內容中性且與系統實際功能相符），標記為已定稿。
- [ ] ABT-S04：**使命與定位／理論框架／利害關係人三頁待定稿。** 佔位文案刻意寫成
  「這裡應該放什麼」而非一段可直接使用的文字：生成看起來可信的公司使命宣言比留白更危險 ——
  留白會被補上，看起來完成的東西不會。三頁均標示「請勿對外引用」。

- [x] TST-S21：新增 `tests/test_venues_and_about.py`（36 項：佔位不可預約、佔位不承諾免費體驗、
  無可用場域時不顯示額度、888 時間軸等於分析模組定義、未定稿分頁必須帶旗標、
...
## 43. Batch S13 稽核軌跡（FND-06，2026-08-20）

- [x] AUD-S01：新增 `backend/reibi_audit.py`。`audit_logs` 表與一個寫入函式早就存在，
  但**只有部門管理（`reibi_batch_f`）在用**。報價、合約、工單、分潤、訂閱的寫入全無紀錄 ——
  問「這筆分潤是誰、在什麼時候確認匯款的」，資料庫裡查不到答案。
- [x] AUD-S02：接上 **11 個寫入點**。金流五項最關鍵：匯款沖帳、發票狀態、
  分潤確認出帳、分潤標記匯款、訂閱審核與重新發碼。
- [x] AUD-S03：報價／合約／工單的狀態轉移**只接一處** —— 三者共用 `update_scoped_status()`，
  因此稽核也集中在那裡，不必在三個端點各寫一次、也不會有人日後新增文件類型時忘記接。
  另接合約簽署快照、報價轉合約、工單驗收三個非狀態轉移的重要操作。
- [x] AUD-S04：**稽核寫入失敗不得回滾業務寫入。** 沒記到 log 是遺憾，
  把已完成的分潤匯款倒回去是災難。失敗只記進應用 log（經 §42 的遮蔽）。
- [x] AUD-S05：`detail` 一律先經 `redact()`。稽核記的是「誰對哪筆單做了什麼」，
  不該把健康資料或憑證抄進另一張表。
- [x] AUD-S06：動作代碼集中定義為 `ACTION_*` 常數（`quote.status`、`commission.paid` 等），
  格式為 `域.動作`。散落的字串會讓日後依動作查詢或告警失去穩定的鍵。
