...
- [ ] MP-06L 個人訂閱功能閘門（Artifact `SubscribeScreen` 與 `isPro`）⚠️ **影響營收**。`MP-06G` 是 L5 端的申請／審核／啟用碼／到期，已完成；**個人用戶端的閘門完全沒有實作**，前端搜尋 `isPro`、`isFreeIndividual`、「訂閱版」皆零結果，免費個人用戶目前拿得到全部功能。Artifact 的限制為：AI 個人報告與年度改善追蹤僅訂閱版可用、免費版歷史趨勢只顯示最近 3 個月、到期前 30 天提醒、到期自動降級但保留資料。此項先前只以測試條目 `MP-01D` 側面提及，從未列為實作待辦 —— 需先確認商業模式：實作，或若已改為純企業採購制則標記 `[N/A]`。
