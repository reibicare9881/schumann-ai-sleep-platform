...
### 2.5.1 正式環境實測（2026-08-22，含一個只有實測才抓得到的 bug）

migration 已套用至遠端（21/21 同步），套用前先取 `backups/schema-before-migration-20260822.sql`
（4,912 行）作為回滾依據。部署後在 staging 實際打 API 驗證，**第一次驗證失敗**：

| 輪次 | 結果 |
|---|---|
| 第一次 | 連續 7 次錯誤通行碼**全部回 401，節流未生效** |
| 診斷 | 查 `reibi_org_login_attempts`：7 筆都有記錄，但出現 **5 個不同的 `ip_hash`** |
| 原因 | Railway 在反向代理後方，`request.client.host` 拿到的是邊緣節點位址且**每次請求都不同**，IP 這層永遠累積不到門檻 |
| 修正 | 改讀 `X-Forwarded-For` 最左端（commit `55fb040`） |
| 第二次 | 前 5 次 401、**第 6 次起 429**，訊息「通行碼錯誤次數過多，請 30 分鐘後再試」；同來源不同角色仍為 401（角色隔離正確） |

**本機測試抓不到這個 bug** —— `TestClient` 永遠回報同一個 `testclient`，怎麼測都會通過。
已補上 6 項針對來源位址取得方式的測試，把「代理每次換位址」的情境釘住。

**一併實測澄清了一件事**：`X-Forwarded-For` 一般被視為呼叫端可偽造，但實測帶著
`X-Forwarded-For: 198.51.100.222` 送出的請求，記錄下來的指紋與不帶標頭時**完全相同** ——
Railway 邊緣會覆寫該標頭，所以這一層在目前部署上偽造不了。但這是**部署環境給的保證，
不是程式碼給的保證**：若日後換到不覆寫該標頭的環境，這層會退化成可繞過，屆時由不看
位址的第二層承接。

驗證用的 14 筆合成紀錄已清除，測試單位 `ABC1` 已確認恢復可正常登入（回 401 而非 429）。

✅ **部署前必做（已完成）**：`20260822120000` 已套用至遠端，本機與遠端同為 21 個 migration。
