...
def client_ip(request: Any) -> Optional[str]:
    ...
    X-Forwarded-For 一般被視為呼叫端可偽造，但 **Railway 的邊緣會覆蓋呼叫端送來的值**：
    2026-08-22 在 staging 實測，帶著 `X-Forwarded-For: 198.51.100.222` 送出的請求，
    記錄下來的指紋與不帶標頭時完全相同，也就是這一層在目前的部署上偽造不了。
    這是**部署環境給的保證，不是這段程式碼的保證** —— 若日後移到不會覆寫該標頭的
    環境（或直接對外開埠），這一層就會退化成可繞過。第二層（單位層跨 IP 計數）
    不看位址，正是為了在那種情況下仍然守得住。
    """
    forwarded = None
