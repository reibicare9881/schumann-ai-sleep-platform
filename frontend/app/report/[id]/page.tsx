...
const buildPDF = (report: any, session: any) => {
  ...
  const b = (s: number, max: number) => {
    ...
    return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:3px">
      <div style="flex:1;height:5px;background:#e8ddd4;border-radius:3px">
        <div style="width:${p}%;height:100%;background:${c};border-radius:3px"></div>
      </div>
      <span style="font-size:10px;color:${c};font-weight:700;min-width:26px">${s}/${max}</span>
    </div>`;
  };
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .st { font-size: 15px; font-weight: 800; color: #217a8c; border-bottom: 2.5px solid #e8ddd4; padding-bottom: 8px; margin-bottom: 18px; letter-spacing: 1px; }
    .sec { margin-bottom: 32px; padding: 0 2px; }
    .np { position: fixed; top: 10px; right: 10px; z-index: 99; display: flex; gap: 7px; }
    .divider { border-top: 1.5px dashed #e8ddd4; margin: 28px 0 18px 0; }
    .table-title { font-size: 12px; color: #6a5e58; font-weight: 700; margin-bottom: 6px; }
    .score-table td { padding: 4px 8px; font-size: 11px; }
    .score-table tr { border-bottom: 1px solid #f0ece8; }
    .rec-block { margin-bottom: 13px; padding: 12px 16px; border-left: 5px solid var(--rec-color,#2a7d8c); background: #faf8f5; border-radius: 0 10px 10px 0; box-shadow: 0 1px 4px #0001; }
    .rec-title { font-size: 12px; font-weight: 700; color: var(--rec-color,#2a7d8c); margin-bottom: 4px; letter-spacing: 0.5px; }
    .rec-content { font-size: 12px; color: #4a3c38; line-height: 1.85; margin: 0; }
    .hero { background: linear-gradient(135deg,#2a7d8c,#1a6474); color: #fff; padding: 44px 36px 36px 36px; border-radius: 14px; margin-bottom: 32px; text-align: center; box-shadow: 0 2px 12px #0002; }
    .hero-grid { display: inline-grid; grid-template-columns: repeat(5,auto); gap: 10px 22px; background: rgba(255,255,255,.13); border-radius: 10px; padding: 13px 20px; margin-bottom: 18px; }
    .hero-score { display: flex; justify-content: center; gap: 18px; flex-wrap: wrap; }
    .score-card { background: #fff; border: 2.5px solid #e8ddd4; border-radius: 10px; padding: 12px 18px; text-align: center; min-width: 100px; box-shadow: 0 1px 6px #0001; }
    .score-label { font-size: 10px; color: #6a5e58; margin-bottom: 2px; font-weight: 700; }
    .score-value { font-size: 22px; font-weight: 800; margin-bottom: 2px; }
    .score-level { font-size: 12px; font-weight: 700; }
    .footer { margin-top: 32px; padding-top: 12px; border-top: 1.5px solid #e8ddd4; }
  </style>
</head>
<body>
  <!-- 列印按鈕 (PDF 內部用) -->
  ...
  <!-- 英雄區塊 -->
  <div class="hero">
    <div style="font-size:10px;letter-spacing:2.5px;opacity:.7;margin-bottom:8px">健康自主管理評估報告 · REIBI Health Platform</div>
    <h1 style="font-size:26px;font-weight:800;margin-bottom:6px;letter-spacing:1.5px">睡眠 × 疼痛聯合評估報告</h1>
    <p style="opacity:.8;font-size:12px;margin-bottom:18px">麗媚生化科技 REIBI · ISI &amp; BPI 國際量表 · 四色燈號分級</p>
    <!-- 基本資訊網格 -->
    <div class="hero-grid">
      ${[
        ...
        <div style="text-align:center">
          <div style="font-size:9px;opacity:.65;margin-bottom:2px">${l}</div>
    ...
    <div class="hero-score">
      ${[
        ...
        <div class="score-card" style="border-color:${LX[x.lv.key as keyof typeof LX]?.br}">
          ...
          <div class="score-value" style="color:${lc[x.lv.key]}">${x.s}<span style="font-size:12px;color:#b8afa8">/${x.m}</span></div>
  ...
  <div class="sec">
    ...
    <div style="background:#fdf6e8;border:1.5px solid #f0c76a;border-radius:10px;padding:13px 18px;margin-bottom:2px">
      <p style="font-size:12px;color:#6b4a10;margin:0;letter-spacing:0.2px">✅ 本人已聲明所填資訊為真實健康狀況，理解此評估目的為健康自主管理促進，同意資料依本系統隱私政策處理。（${declarationTs ? new Date(declarationTs).toLocaleString("zh-TW") : "—"}）</p>
    </div>
  </div>
  ...
  <div class="sec">
    ...
    <div style="display:flex;gap:14px;padding:13px 16px;background:${LX[sLevel.key as keyof typeof LX]?.bg};border:2px solid ${LX[sLevel.key as keyof typeof LX]?.br};border-radius:12px;margin-bottom:12px;align-items:center">
      <div style="text-align:center;min-width:52px">
        ${["red", "orange", "yellow", "green"].map(k => `
          <div style="width:16px;height:16px;border-radius:50%;margin:3px auto;background:${k === sLevel.key ? lc[k] : "#e8ddd4"}"></div>
        `).join("")}
      </div>
      <div style="text-align:left">
        <div style="font-size:18px;font-weight:800;color:${lc[sLevel.key]};margin-bottom:2px">${LL[sLevel.key as keyof typeof LL]} ${sLevel.label}</div>
        <p style="font-size:12px;color:#4a3c38;margin:3px 0 2px 0">${sLevel.desc || ""}</p>
        <p style="font-size:12px;color:${lc[sLevel.key]};font-weight:700">▸ ${sLevel.action || ""}</p>
      </div>
    </div>
  ...
  <div class="sec">
    ...
    <div style="display:flex;gap:14px;padding:13px 16px;background:${LX[pLevel.key as keyof typeof LX]?.bg};border:2px solid ${LX[pLevel.key as keyof typeof LX]?.br};border-radius:12px;margin-bottom:12px;align-items:center">
      <div style="text-align:center;min-width:52px">
        ${["red", "orange", "yellow", "green"].map(k => `
          <div style="width:16px;height:16px;border-radius:50%;margin:3px auto;background:${k === pLevel.key ? lc[k] : "#e8ddd4"}"></div>
        `).join("")}
      </div>
      <div style="text-align:left">
        <div style="font-size:18px;font-weight:800;color:${lc[pLevel.key]};margin-bottom:2px">${LL[pLevel.key as keyof typeof LL]} ${pLevel.label}</div>
        <p style="font-size:12px;color:#4a3c38;margin:3px 0 2px 0">${pLevel.desc || ""}</p>
        <p style="font-size:12px;color:${lc[pLevel.key]};font-weight:700">▸ ${pLevel.action || ""}</p>
      </div>
    </div>
  ...
  <div class="sec">
    ...
    <div style="background:linear-gradient(135deg,#ebf6f8,#eef0fb);border:1.5px solid #b8dfe5;border-radius:10px;padding:13px 18px;margin-top:10px;box-shadow:0 1px 4px #0001;">
      <div style="font-size:12px;font-weight:700;color:#1a6074;margin-bottom:4px">🔬 麗媚生化科技 REIBI · 舒曼波 &amp; LA200 雷射治療</div>
      <p style="font-size:12px;color:#2c4a52;line-height:1.85;margin:0">${recs.reibiProducts || ""}</p>
    </div>
  </div>
  <div class="divider"></div>
  ...
  <!-- 頁尾 -->
  <div class="footer">
    <p style="font-size:9px;color:#b8afa8;margin:0 0 5px 0">[1] 衛福部（2023）。2023–2026年國民健康白皮書。[2] Morin CM (1993). ISI. Guilford Press. [3] Cleeland CS (1994). BPI. Ann Acad Med.</p>
    <p style="font-size:9px;color:#c05a28;font-weight:700;margin:0 0 5px 0">⚠ 本報告僅供健康篩查，不能替代專業醫療診斷。</p>
    <p style="font-size:9px;color:#d0c8c0;text-align:center;margin-top:10px">報告編號：${id.toUpperCase()} · ${date} · 麗媚生化科技 REIBI健康管理系統</p>
  </div>
</body>
</html>`;
};
