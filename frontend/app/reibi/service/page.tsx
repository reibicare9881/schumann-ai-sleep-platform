...
export default function ReibiServicePage() {
  ...
  const load = useCallback(async () => {
    ...
    await run(async () => {
      ...
      let initialEnterpriseId = "";
      if (nextScope.requires_enterprise && nextScope.enterprises?.length === 1) {
        initialEnterpriseId = String(nextScope.enterprises[0].id);
        setTicket(current => ({ ...current, enterprise_id: initialEnterpriseId }));
      }
      setTickets(unwrap(ticketsResponse, "無法讀取服務案件") || []);
      setAnnouncements(unwrap(announcementsResponse, "無法讀取公告") || []);
      if (isManager && (!isSuper || initialEnterpriseId)) {
        setArchitecture(unwrap(await API.getReibiArchitecture(initialEnterpriseId ? Number(initialEnterpriseId) : undefined), "無法讀取部門架構"));
      }
      if (isSuper) {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 px-4 py-8">
    ...
    {tab === "tickets" && <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      <form className={card} onSubmit={event => { event.preventDefault(); void submitTicket(); }}><h2 className="flex items-center gap-2 font-black"><ClipboardList className="h-5 w-5 text-sky-700" />新增服務案件</h2>
        {serviceScope.requires_enterprise && <label className="mt-4 block text-xs font-bold">服務企業<select required className={input} value={ticket.enterprise_id} onChange={e => { const enterpriseId = e.target.value; setTicket({ ...ticket, enterprise_id: enterpriseId }); if (isManager) void run(async () => setArchitecture(enterpriseId ? unwrap(await API.getReibiArchitecture(Number(enterpriseId)), "無法讀取部門架構") : null)); }}><option value="">請選擇企業</option>{(serviceScope.enterprises || []).map((row: Row) => <option key={row.id} value={row.id}>{row.org_name}（{row.org_code}）</option>)}</select></label>}
