...
import { AlertTriangle, ArrowLeft, BellRing, BookOpen, CheckCircle2, ClipboardList, MapPinned, RefreshCw, ShieldAlert, TrendingUp } from "lucide-react";
...
const workflowLabels: Record<string, { title: string; metrics: Record<string, string>; href: string }> = {
  ...
  service_tickets: { title: "服務案件", metrics: { pending: "待處理", total: "全部" }, href: "/reibi/service" },
};
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <header className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap gap-2">{["reibi_super", "reibi_finance"].includes(session.systemRole) && <Link href="/reibi/onboarding" className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white"><ClipboardList className="h-4 w-4" />新案開通</Link>}{["reibi_super", "reibi_data"].includes(session.systemRole) && <Link href="/reibi/l5/regions" className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700"><MapPinned className="h-4 w-4" />區域佈點</Link>}<Link href="/reibi/l5/manual" className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700"><BookOpen className="h-4 w-4" />操作手冊</Link><button onClick={() => void load()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> 重新整理</button></div>
      </header>
